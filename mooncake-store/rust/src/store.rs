...
// Raw FFI layer. Exactly one backend is active and both expose the same items
// (`mooncake_store_t`, `mooncake_replicate_config_t`, `mooncake_store_*`), so
// the safe wrapper below is backend-agnostic.
//
// `link`: bindgen bindings, statically linked (build.rs). `dlopen`: runtime
// libloading bindings (ffi_dlopen.rs).
#[cfg(feature = "link")]
#[allow(dead_code)]
mod ffi {
...
impl MooncakeStore {
    ...
    pub fn new() -> Result<Self, StoreError> {
        // dlopen backend: resolve libmooncake_store.so before the first C call.
        #[cfg(all(feature = "dlopen", not(feature = "link")))]
        ffi::ensure_loaded()?;
...
mod tests {
    ...
    // These construct a live store handle, so they need the library: statically
    // linked under `link`, or present at run time under `dlopen`.
    #[cfg(feature = "link")]
    #[test]
